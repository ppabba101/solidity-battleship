...
export interface GameShellProps {
   ...
   * `/real/g/:gameId`). Used by the join flow in WP4+. The local hot-seat
   ...
   * (local anvil). WP4+ pipes the mode-scoped address via
   * `useActiveContractAddress()`.
   */
  contractAddress?: `0x${string}`;
}

export function GameShell({ mode: _mode, urlGameId: _urlGameId, contractAddress: _contractAddress }: GameShellProps) {
  // WP3 scope: accept props so the router surface compiles; consumption of
  // mode / urlGameId / contractAddress is threaded through in WP4+.
  void _mode;
  void _urlGameId;
